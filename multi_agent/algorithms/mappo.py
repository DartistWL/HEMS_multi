...
class PopArtNormalizer:
    ...
    def update(self, targets):
        ...
        if self.running_count == 0:
            ...
            new_var_value = max(1e-8, old_var + alpha * delta_var)  # 使用max而不是torch.clamp（因为值是float）
    ...
    def denormalize(self, normalized_values):
        ...
        denormalized = normalized_values * (self.std + 1e-8) + self.mean
        return denormalized
    ...
    def _initialize_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, nn.Linear):
                ...
                nn.init.xavier_uniform_(m.weight, gain=0.1)  # 使用较小的gain值
...
class MAPPO:
    ...
    def __init__(self, env, n_agents=3, local_state_dim=None, global_state_dim=None,
        ...
        if local_state_dim is None:
            # 从环境获取局部状态维度（使用env.reset()以包含社区信息）
            sample_local_states = env.reset(mode='train', date_index=0)
            sample_local_state = sample_local_states[0]  # 使用第一个智能体的状态
    ...
    def select_actions(self, local_states, deterministic=False):
        ...
        Args:
            local_states: 局部状态列表（每个智能体一个）
            deterministic: 是否使用确定性策略（评估时使用）
        
        Returns:
            actions: 动作列表（每个智能体一个动作字典）
            action_log_probs: 动作的log概率列表
        """
        actions = []
        action_log_probs = []
    ...
    def store_transition(self, local_states, global_state, actions, action_log_probs,
        ...
        if self.use_reward_normalization and self.reward_running_stats is not None:
            # 将所有智能体的奖励转换为tensor并更新统计
            # 使用所有智能体的奖励来更新运行统计（joint reward的统计）
            ...
            # 对每个智能体的奖励分别进行z-score标准化（使用相同的统计信息）
            if self.reward_running_stats.count > 0:
                ...
                for i, reward in enumerate(rewards):
                    normalized_rewards[i] = (reward - mean) / std
        
        # 转换为向量
        local_state_vectors = [self._state_dict_to_vector(s) for s in local_states]
        next_local_state_vectors = [self._state_dict_to_vector(s) for s in next_local_states]
        ...
        if self.use_state_normalization and self.local_running_stats is not None:
            ...
            # 归一化局部状态
            normalized_local_state_vectors = [
                self._normalize_local_state(lv) for lv in local_state_vectors
            ]
            normalized_next_local_state_vectors = [
                self._normalize_local_state(nlv) for nlv in next_local_state_vectors
            ]
            
            # 拼接归一化后的局部状态作为Critic输入
            global_state_vector = np.concatenate(normalized_local_state_vectors)
            next_global_state_vector = np.concatenate(normalized_next_local_state_vectors)
        else:
            # 不归一化，直接拼接
            global_state_vector = np.concatenate(local_state_vectors)
            next_global_state_vector = np.concatenate(next_local_state_vectors)
            normalized_local_state_vectors = local_state_vectors
            normalized_next_local_state_vectors = next_local_state_vectors
        ...
        for agent_id in range(self.n_agents):
            self.buffer['local_states'][agent_id].append(normalized_local_state_vectors[agent_id])
            self.buffer['actions'][agent_id].append(actions[agent_id])
            self.buffer['rewards'][agent_id].append(normalized_rewards[agent_id])
            self.buffer['next_local_states'][agent_id].append(normalized_next_local_state_vectors[agent_id])
    ...
    def update(self):
        ...
        # 准备数据
        buffer_size = len(self.buffer['global_states'])
        
        # 转换全局状态（已经在store_transition中归一化，直接使用）
        global_states = np.array(self.buffer['global_states'])
        next_global_states = np.array(self.buffer['next_global_states'])
        
        # 全局状态已经在store_transition中归一化（通过局部状态归一化后拼接），不需要再次归一化
        ...
        if not self.buffer['dones'][0][-1]:
            # 全局状态已经在store_transition中归一化，直接使用
        ...
        # 更新Critic（所有智能体共享，使用所有智能体的经验）
        # 计算所有智能体的平均returns作为Critic的目标
        all_returns = []
        for agent_id in range(self.n_agents):
            rewards = np.array(self.buffer['rewards'][agent_id])
            if self.reward_scale != 1.0:
        ...
        if self.use_popart:
            ...
            returns_tensor_critic_normalized = self.popart_normalizer.normalize(returns_tensor_critic)
        else:
            returns_tensor_critic_normalized = returns_tensor_critic
        
        for epoch in range(self.epochs):
            values_pred = self.critic(global_states_tensor).squeeze()
            
            # Critic预测归一化后的值，根据配置选择损失函数
            if self.critic_loss_type == 'huber':
                critic_loss = F.smooth_l1_loss(values_pred, returns_tensor_critic_normalized, beta=self.huber_delta)
            else:  # 默认使用MSE
                critic_loss = F.mse_loss(values_pred, returns_tensor_critic_normalized)
    ...
    def save(self, save_dir):
        ...
        for agent_id, actor in enumerate(self.actors):
            torch.save(actor.state_dict(), 
                      os.path.join(save_dir, f'mappo_actor_{agent_id}.pth'))
        
        # 保存Critic网络
        torch.save(self.critic.state_dict(),
                  os.path.join(save_dir, 'mappo_critic.pth'))
