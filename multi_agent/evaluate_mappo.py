...
def _resolve_mappo_model_dir(model_dir, config_path='multi_agent/config.json'):
    """根据 config 的 credit_pricing.scheme 解析 MAPPO 模型目录（评估时自动用对应模型，不覆盖原模型）。"""
...
def evaluate_mappo(model_dir='multi_agent/algorithms/models', baseline_peak=31.01, 
    ...
    """
    评估MAPPO模型。若 model_dir 为默认且 config 中 credit_pricing.scheme=contribution_based，则自动加载 models_contribution_based 目录。
    ...
    for episode in range(num_episodes):
        ...
        print(f"Episode {episode + 1}/{num_episodes} | "
              f"Date: {date} (PV={pv_coeff[0]:.1f}) | "
              f"Return: {episode_return:.2f} | "
              f"Cost: {episode_cost:.2f} | "
              f"Peak: {episode_peak_load:.2f} kW")
