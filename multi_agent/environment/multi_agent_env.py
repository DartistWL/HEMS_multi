...
class MultiAgentHEMEnv:
    ...
    def reset(self, mode='train', date_index=0, house_index=None, initial_community_soc=None):
        ...
        pv_list = getattr(self, 'evaluation_pv_coefficients_list', None) if mode != 'train' else getattr(self, 'training_pv_coefficients_list', None)
    ...
    def step(self, actions):
        ...
        max_energy_charged = max(self._agent_energy_charged_to_community.values()) if self._agent_energy_charged_to_community else 0.0
        ...
        for i, action in enumerate(actions):
            ...
            if abs(actual_power) > 1e-6:  # 避免浮点误差
                ...
                if transaction_type == 'sell':
                    self._agent_energy_charged_to_community[i] = self._agent_energy_charged_to_community.get(i, 0.0) + energy_amount
        ...
        peak_penalty, threshold, is_peak = self.peak_tracker.calculate_peak_penalty(community_net_load, use_normalization=False)
        ...
        self._charge_power_stats['max_total_charge'] = max(self._charge_power_stats['max_total_charge'], total_charge_power_all)
