...
def train_mappo(num_episodes=1000, baseline_peak=None, save_dir='multi_agent/algorithms/models',
    ...
    full_config = load_config()  # 或者 load_config('config.json')
    config = full_config.get('training', {}) if isinstance(full_config, dict) and full_config else {}
    credit_scheme = (full_config.get('credit_pricing') or {}).get('scheme', 'uniform') if isinstance(full_config,
                                                                                                     dict) else 'uniform'
    ...
    # 在创建 MAPPO 之前，正确计算状态维度
    from multi_agent.algorithms.action_utils import state_dict_to_vector

    sample_states = env.reset(mode='train', date_index=0, house_index=0)
    sample_local_state = sample_states[0]
    local_state_dim = len(state_dict_to_vector(sample_local_state))
    ...
    mappo = MAPPO(
        ...
        n_agents=3,
        device='cpu',  # 强制 CPU
        local_state_dim=local_state_dim,  # 新增
        global_state_dim=global_state_dim,  # 新增
        hidden_dim=128,
        gamma=0.96,
        lmbda=0.95,
        eps=0.2,
        ...
        lr_critic=1e-4,
        # batch_size=64,
        batch_size=32,  # 降低 batch_size
        ent_coef=ent_coef,
        max_grad_norm=1.0,
        # use_state_normalization=use_state_norm,
        # use_popart=use_popart,
        use_state_normalization=False,  # 关闭状态归一化
        use_popart=False,  # 关闭 Pop-Art
        reward_scale=1.0,
        # use_reward_normalization=use_reward_norm,
        use_reward_normalization=False,  # 关闭奖励归一化
    ...
    num_train_houses = len(train_house_ids)
    ...
    for episode in range(num_episodes):
        ...
        # 强制验证 buffer 结构（可选）
        for i in range(3):
            if not isinstance(mappo.buffer['rewards'][i], list):
                print(f"CRITICAL: buffer rewards[{i}] is {type(mappo.buffer['rewards'][i])}, fixing...")
                mappo.buffer['rewards'][i] = []
        ...
        if (episode + 1) % 10 == 0:
            ...
            if hasattr(env, '_reward_breakdown') and len(env._reward_breakdown['total_reward']) > 0:
                ...
                avg_peak_pen = np.mean(breakdown['peak_penalty_value'][-n_steps:]) if breakdown[
                    'peak_penalty_value'] else 0
                avg_queue_pen = np.mean(breakdown['queue_penalty'][-n_steps:]) if breakdown['queue_penalty'] else 0
                avg_ess_reward = np.mean(breakdown['community_ess_reward'][-n_steps:]) if breakdown[
                    'community_ess_reward'] else 0
                ...
                if charge_stats and len(charge_stats['total_charge_power_all']) > 0:
                    ...
                    avg_charge = np.mean(charge_stats['total_charge_power_all'][-n_steps:]) if len(
                        charge_stats['total_charge_power_all']) >= n_steps else np.mean(
                        charge_stats['total_charge_power_all'])
...
def evaluate_mappo(mappo, baseline_peak=31.01, num_episodes=10, mode='eval'):
    """
    评估MAPPO算法（使用测试住户和2020年日期）
    """
    # 定义测试住户（与训练一致：H16~H20）
