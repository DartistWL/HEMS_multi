...
def plot_net_load_stacked(independent_data_file=None, rule_based_data_file=None, 
    ...
    for method in ['independent', 'rule_based', 'mappo']:
        ...
        # 获取PV流向信息（如果可用）
        # 注意：目前数据收集可能不包含这些详细信息，我们需要从基本数据推断
        # 或者扩展数据收集脚本来收集这些信息
        ...
        if len(community_net_loads) > 0:
            ...
            # 计算社区总负荷（从电网购电的部分）
            # 总负荷 = 所有家庭负荷 - PV直接使用（但PV直接使用已经在净负荷中考虑了）
            # 实际上，community_net_loads 已经是三户家庭的净负荷之和，已经考虑了PV、ESS、EV等
            # 但我们仍然可以展示分解
            
            # 计算总需求（从电网购电的正向堆叠）
            # 注意：这里我们需要计算实际从电网购电的部分
            # 由于PV已经直接抵消了部分负荷，我们显示的是净需求
            # 但为了堆叠图的清晰，我们显示总负荷（家庭基础负荷等）
...
def main():
    ...
    # 如果提供了comparison_data，尝试从中提取数据
    if args.comparison_data and os.path.exists(args.comparison_data):
        # 这里需要根据comparison_data的结构来解析
        # 但comparison_data通常只包含summary，不包含详细的时间序列数据
        # 所以优先使用单独的数据文件
        pass
    ...
    if not independent_file and os.path.exists(os.path.join(visualization_data_dir, 'independent_data.json')):
        independent_file = os.path.join(visualization_data_dir, 'independent_data.json')
    if not rule_based_file and os.path.exists(os.path.join(visualization_data_dir, 'rule_based_data.json')):
        rule_based_file = os.path.join(visualization_data_dir, 'rule_based_data.json')
    if not mappo_file and os.path.exists(os.path.join(visualization_data_dir, 'mappo_data.json')):
        mappo_file = os.path.join(visualization_data_dir, 'mappo_data.json')
